...
public class SokoTurtleBot extends Sokoban2Domain {
	...
	public static final String				ACTIONROTATECCW = "rotate_ccw";
	 ...
	 * Class that rotates clockwise.
	 */
	public static class RotateActionCCW extends Action{


		public RotateActionCCW(Domain domain){
			super(ACTIONROTATECCW, domain, "");
		}
	...
	public static class MoveForwardAction extends Action{
		...
		protected State performActionHelper(State s, String[] params) {
			...
			int dxdelta = 2*xdelta;
			int dydelta = 2*ydelta;
			int n2x = ax+dxdelta;
			int n2y = ay+dydelta;

			boolean permissibleMove = false;
			ObjectInstance pushedBlock = blockAtPoint(s, nx, ny);
			ObjectInstance pushedBlock2 = blockAtPoint(s, n2x, n2y);
			if(pushedBlock != null){
			...
			else if(pushedBlock2 != null){
				int bx = pushedBlock2.getDiscValForAttribute(ATTX);
				int by = pushedBlock2.getDiscValForAttribute(ATTY);
				...
				if(!wallAt(s, roomContaining, nbx, nby) && blockAtPoint(s, nbx, nby) == null){
					...
					//move the block
					pushedBlock2.setValue(ATTX, nbx);
					pushedBlock2.setValue(ATTY, nby);
